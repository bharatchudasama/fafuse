...
class DeiT(VisionTransformer):
    ...
    def forward(self, x):
        ...
        # x is a tensor of patch embeddings, pe is the positional embedding
        pe = self.pos_embed
        x = x + pe
        x = self.pos_drop(x)
...
def deit_small_patch16_224(pretrained=False, **kwargs):
    model = DeiT(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),img_size=384,**kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        ckpt = torch.load('pretrained/deit_base_patch16_384-8de9b5d1.pth')
        model.load_state_dict(ckpt['model'], strict=False)
    
    # This part of the code resizes the positional embedding (pe)
    pe = model.pos_embed[:, 1:, :].detach()
    pe = pe.transpose(-1, -2)
    pe = pe.view(pe.shape[0], pe.shape[1], int(np.sqrt(pe.shape[2])), int(np.sqrt(pe.shape[2])))
    
    # --- FIX: Changed size from (24,24) to (14,14) ---
    # This will resize the positional embedding to match the 224x224 input size
    # (224 / 16 = 14)
