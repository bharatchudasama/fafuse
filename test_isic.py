...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='snapshots\FAFuse_B\epoch_9.pth')
    ...
    for i in range(test_loader.size):
        ...
        if opt.save_path is not None:
            # Convert both arrays to uint8 before saving
            pred_img = (res * 255).astype(np.uint8)
            gt_img = (gt * 255).astype(np.uint8)

            imageio.imwrite(opt.save_path+'/'+str(i)+'_pred.jpg', pred_img)
            imageio.imwrite(opt.save_path+'/'+str(i)+'_gt.jpg', gt_img)
