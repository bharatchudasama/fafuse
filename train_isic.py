...
def train(train_loader, model, optimizer, epoch, best_loss):
    ...
    print(f"\n--- Starting Epoch {epoch}/{opt.epoch} ---")
    ...
    meanloss = test(model, opt.test_path)  # FIXED - call test before saving
    ...
    print(f'🎯 New loss: {meanloss}')
    if meanloss > best_loss:
        print(f'🎯 New best loss: {meanloss}')
        best_loss = meanloss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_epoch', type=int, default=1, help='epoch to start from (used when resuming)')
    ...
    if opt.resume is not None and os.path.isfile(opt.resume):
        ...
        best_loss = checkpoint.get('best_loss', 0)
    else:
        best_loss = 0
